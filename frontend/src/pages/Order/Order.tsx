...
const Order: FC = (): ReactElement => {
    ...
    const onFormSubmit = (order: OrderFormData): void => {
        order.email = localStorage.getItem("email") === null ? "" : (localStorage.getItem("email") as string);
        const input: (string | null)[][] = JSON.parse(localStorage.getItem("perfumes") as string);

        // Convert to list and remove items with null
        const filteredList: [string, number][] = input.filter((item) => item[0] !== null) as [string, number][];

        const keyValueObject: { [key: string]: number } = filteredList.reduce((acc, [key, value]) => {
            acc[key] = value;
            return acc;
        }, {} as { [key: string]: number });
        dispatch(addOrder({ order: { ...order, productId: keyValueObject, totalPrice }, history }));
    };
                        ...
                        <Row gutter={[32, 32]}>
                            {perfumes.map((perfume) => (
                                <OrderItem
                                    ...
                                    perfume={perfume}
                                    quantity={perfumesFromLocalStorage.get(perfume.productId)}
                                />
                            ))}
                        </Row>
