...
export const cartSlice = createSlice({
    ...
    reducers: {
        ...
        removePerfumeById(state, action: PayloadAction<number>) {
            const perfumes = state.perfumes.filter((perfume) => perfume.productId !== action.payload);
...
const calculatePrice = (perfumes: Array<ProductResponse>): number => {
    ...
    perfumesFromLocalStorage.forEach((value, key) => {
        const perfume = perfumes.find((perfume) => perfume.productId === key);
